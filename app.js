...
function translateTechTerm() {
    ...
    var dictionary = {
        ...
        'deploy': 'Развёртывание — процесс публикации готового сайта или приложения в интернет, чтобы пользователи могли им пользоваться.',
        ...
        'bug': 'Ошибка в программе — когда что-то работает не так, как задумано. Как опечатка в тексте, только в коде.',
        'фронтенд': 'Внешняя часть сайта — то, что видит пользователь: кнопки, формы, анимации. Как фасад здания.',
        'frontend': 'Внешняя часть сайта — то, что видит пользователь: кнопки, формы, анимации. Как фасад здания.',
        'бэкенд': 'Внутренняя часть сайта — серверная логика, базы данных, обработка запросов. Как фундамент и коммуникации в здании.',
        'backend': 'Внутренняя часть сайта — серверная логика, базы данных, обработка запросов. Как фундамент и коммуникации в здании.',
        'база данных': 'Хранилище информации — как цифровой шкаф с папками. Хранит всех клиентов, заказы, товары и т.д.',
        'database': 'Хранилище информации — как цифровой шкаф с папками. Хранит всех клиентов, заказы, товары и т.д.',
        'адаптив': 'Адаптивная вёрстка — когда сайт автоматически подстраивается под размер экрана: телефон, планшет, компьютер.',
        'responsive': 'Адаптивная вёрстка — когда сайт автоматически подстраивается под размер экрана: телефон, планшет, компьютер.',
        'хостинг': 'Аренда места на сервере в интернете, где хранится твой сайт. Как аренда офиса, только цифрового.',
        'hosting': 'Аренда места на сервере в интернете, где хранится твой сайт. Как аренда офиса, только цифрового.',
        'домен': 'Имя сайта в интернете (например, google.com). Как адрес дома, только для веб-страницы.',
        'domain': 'Имя сайта в интернете (например, google.com). Как адрес дома, только для веб-страницы.',
        ...
        'cache': 'Временное хранилище данных для ускорения работы. Как заметки на столе, чтобы не бегать каждый раз в шкаф.',
        ...
        'commit': 'Сохранение изменений в Git. Как снимок текущего состояния проекта.',
        'пул-реквест': 'Запрос на внесение изменений в код. Как предложение коллеге: "Посмотри, что я сделал, одобришь?"',
        'pull request': 'Запрос на внесение изменений в код. Как предложение коллеге: "Посмотри, что я сделал, одобришь?"',
        'тестирование': 'Проверка программы на ошибки — как техосмотр автомобиля перед продажей.',
        'testing': 'Проверка программы на ошибки — как техосмотр автомобиля перед продажей.',
        'оптимизация': 'Ускорение работы сайта или программы. Как тюнинг двигателя — едет быстрее, расходует меньше.',
        'optimization': 'Ускорение работы сайта или программы. Как тюнинг двигателя — едет быстрее, расходует меньше.',
        'интеграция': 'Соединение разных систем воедино. Как подключение принтера к компьютеру — они начинают работать вместе.',
        'integration': 'Соединение разных систем воедино. Как подключение принтера к компьютеру — они начинают работать вместе.',
        'парсер': 'Программа для автоматического сбора данных с сайтов. Как робот-пылесос, только собирает информацию.',
        'parser': 'Программа для автоматического сбора данных с сайтов. Как робот-пылесос, только собирает информацию.',
        'скрипт': 'Небольшая программа для автоматизации задач. Как инструкция для робота: "делай раз, делай два".',
        'script': 'Небольшая программа для автоматизации задач. Как инструкция для робота: "делай раз, делай два".',
        'верстка': 'Создание внешней оболочки сайта из HTML и CSS. Как украшение торта — делаем красиво и аккуратно.',
        'layout': 'Создание внешней оболочки сайта из HTML и CSS. Как украшение торта — делаем красиво и аккуратно.',
        'сервер': 'Компьютер, который работает 24/7 и обслуживает запросы пользователей. Как официант в ресторане — принимает заказы и приносит блюда.',
        'server': 'Компьютер, который работает 24/7 и обслуживает запросы пользователей. Как официант в ресторане — принимает заказы и приносит блюда.',
        'клиент': 'Программа или браузер, который отправляет запросы на сервер. Как посетитель ресторана — делает заказ.',
        'client': 'Программа или браузер, который отправляет запросы на сервер. Как посетитель ресторана — делает заказ.',
        'запрос': 'Запрос данных от клиента к серверу. Как вопрос: "Дай мне эту страницу".',
        'request': 'Запрос данных от клиента к серверу. Как вопрос: "Дай мне эту страницу".',
        'ответ': 'Данные, которые сервер отправляет клиенту. Как ответ на вопрос.',
        'response': 'Данные, которые сервер отправляет клиенту. Как ответ на вопрос.',
        'json': 'Формат обмена данными — как универсальный язык, который понимают все программы.',
        'xml': 'Формат обмена данными — более сложный, чем JSON, но тоже универсальный.',
        'rest': 'Архитектурный стиль для API — как правила хорошего тона в общении программ.',
        'graphql': 'Язык запросов для API — позволяет клиенту просить именно те данные, которые нужны, без лишнего.',
        'websocket': 'Технология для двусторонней связи в реальном времени. Как телефонный звонок — оба могут говорить одновременно.',
        'токен': 'Цифровой ключ доступа — как пропуск в офис. Даёт право пользоваться системой.',
        'token': 'Цифровой ключ доступа — как пропуск в офис. Даёт право пользоваться системой.',
        'авторизация': 'Проверка прав доступа — как охранник на входе: "А ты точно можешь сюда войти?"',
        'authentication': 'Проверка личности — как паспортный контроль: "Ты действительно тот, за кого себя выдаёшь?"',
        'аутентификация': 'Проверка личности — как паспортный контроль: "Ты действительно тот, за кого себя выдаёшь?"',
        'шифрование': 'Преобразование данных в нечитаемый вид. Как секретный код, который знают только отправитель и получатель.',
        'encryption': 'Преобразование данных в нечитаемый вид. Как секретный код, который знают только отправитель и получатель.',
        'хэш': 'Уникальный "отпечаток пальца" данных. Как контрольная сумма — если данные изменились, хэш тоже изменится.',
        'hash': 'Уникальный "отпечаток пальца" данных. Как контрольная сумма — если данные изменились, хэш тоже изменится.',
        'куки': 'Маленькие файлы, которые сайт сохраняет в браузере. Как заметки: "Этот пользователь уже заходил".',
        'cookies': 'Маленькие файлы, которые сайт сохраняет в браузере. Как заметки: "Этот пользователь уже заходил".',
        'сессия': 'Временной промежуток активности пользователя. Как визит в магазин — зашёл, купил, вышел.',
        'session': 'Временной промежуток активности пользователя. Как визит в магазин — зашёл, купил, вышел.',
        'контейнер': 'Изолированная среда для запуска программы. Как коробка: внутри всё своё, не мешает другим.',
        'container': 'Изолированная среда для запуска программы. Как коробка: внутри всё своё, не мешает другим.',
        'docker': 'Инструмент для создания контейнеров. Как упаковочная машина — упаковывает программу со всем необходимым.',
        'виртуализация': 'Создание виртуальных компьютеров внутри физического. Как квартира в доме — у каждого своя, но дом один.',
        'облако': 'Удалённые серверы для хранения данных и запуска программ. Как облако на небе — вроде далеко, но доступно отовсюду.',
        'cloud': 'Удалённые серверы для хранения данных и запуска программ. Как облако на небе — вроде далеко, но доступно отовсюду.',
        'cdn': 'Сеть серверов по всему миру для быстрой доставки контента. Как филиалы банка — ближе к клиенту = быстрее обслуживание.',
        'базовый': 'Основной, фундаментальный. Как фундамент дома.',
        'продвинутый': 'Улучшенный, с дополнительными возможностями. Как тюнингованная версия.',
        'кастомный': 'Сделанный на заказ, под конкретные требования. Как костюм у портного.',
        'custom': 'Сделанный на заказ, под конкретные требования. Как костюм у портного.',
        'масштабирование': 'Увеличение мощности системы при росте нагрузки. Как расширение дороги при увеличении трафика.',
        'scaling': 'Увеличение мощности системы при росте нагрузки. Как расширение дороги при увеличении трафика.',
        'нагрузка': 'Количество запросов к системе. Как очередь в магазине — чем больше людей, тем выше нагрузка.',
        'load': 'Количество запросов к системе. Как очередь в магазине — чем больше людей, тем выше нагрузка.',
        'производительность': 'Скорость и эффективность работы системы. Как скорость автомобиля.',
        'performance': 'Скорость и эффективность работы системы. Как скорость автомобиля.',
        'безопасность': 'Защита системы от угроз. Как замок на двери.',
        'security': 'Защита системы от угроз. Как замок на двери.',
        'уязвимость': 'Слабое место в защите. Как открытое окно в доме.',
        'vulnerability': 'Слабое место в защите. Как открытое окно в доме.',
        'эксплойт': 'Использование уязвимости для атаки. Как взломщик, который нашёл открытое окно.',
        'exploit': 'Использование уязвимости для атаки. Как взломщик, который нашёл открытое окно.',
        'файрвол': 'Система фильтрации сетевого трафика. Как охранник на входе — проверяет, кого пропустить.',
        'firewall': 'Система фильтрации сетевого трафика. Как охранник на входе — проверяет, кого пропустить.',
        'прокси': 'Посредник между клиентом и сервером. Как секретарь — принимает звонки и передаёт нужному человеку.',
        'proxy': 'Посредник между клиентом и сервером. Как секретарь — принимает звонки и передаёт нужному человеку.',
        'балансировщик': 'Распределитель нагрузки между серверами. Как диспетчер — направляет запросы на свободные серверы.',
        'load balancer': 'Распределитель нагрузки между серверами. Как диспетчер — направляет запросы на свободные серверы.',
        'миграция': 'Перенос данных или системы с одного места на другое. Как переезд в новый офис.',
        'migration': 'Перенос данных или системы с одного места на другое. Как переезд в новый офис.',
        'бэкап': 'Резервная копия данных. Как ксерокопия важного документа — на случай потери оригинала.',
        'backup': 'Резервная копия данных. Как ксерокопия важного документа — на случай потери оригинала.',
        'восстановление': 'Возврат данных из резервной копии. Как восстановление файла из корзины.',
        'recovery': 'Возврат данных из резервной копии. Как восстановление файла из корзины.',
        'автоматизация': 'Выполнение задач без участия человека. Как робот-пылесос — работает сам.',
        'automation': 'Выполнение задач без участия человека. Как робот-пылесос — работает сам.',
        'дебаг': 'Поиск и исправление ошибок в коде. Как детектив — ищет преступника (баг) и обезвреживает его.',
        'debug': 'Поиск и исправление ошибок в коде. Как детектив — ищет преступника (баг) и обезвреживает его.',
        'лог': 'Запись событий в системе. Как дневник — что произошло, когда и с кем.',
        'log': 'Запись событий в системе. Как дневник — что произошло, когда и с кем.',
        'мониторинг': 'Постоянное наблюдение за системой. Как камера видеонаблюдения — следит, всё ли в порядке.',
        'monitoring': 'Постоянное наблюдение за системой. Как камера видеонаблюдения — следит, всё ли в порядке.',
        'алерт': 'Сигнал о проблеме. Как сирена — что-то пошло не так!',
        'alert': 'Сигнал о проблеме. Как сирена — что-то пошло не так!',
        'метрика': 'Измеримый показатель работы системы. Как спидометр — показывает скорость.',
        'metric': 'Измеримый показатель работы системы. Как спидометр — показывает скорость.',
        'дашборд': 'Панель управления с ключевыми показателями. Как приборная панель в машине.',
        'dashboard': 'Панель управления с ключевыми показателями. Как приборная панель в машине.',
        'аналитика': 'Анализ данных для принятия решений. Как статистика в спорте — помогает понять, что работает.',
        'analytics': 'Анализ данных для принятия решений. Как статистика в спорте — помогает понять, что работает.',
        'конверсия': 'Процент пользователей, совершивших целевое действие. Как процент покупателей от всех зашедших в магазин.',
        'conversion': 'Процент пользователей, совершивших целевое действие. Как процент покупателей от всех зашедших в магазин.',
        'трафик': 'Количество посетителей сайта. Как поток людей в магазине.',
        'traffic': 'Количество посетителей сайта. Как поток людей в магазине.',
        'seo': 'Оптимизация сайта для поисковых систем. Как настройка витрины, чтобы её видели прохожие.',
        'семантическое ядро': 'Список ключевых слов, по которым сайт должен находиться в поиске.',
        'лендинг': 'Одностраничный сайт для конкретной цели. Как рекламный щит — одна задача, одно действие.',
        'landing': 'Одностраничный сайт для конкретной цели. Как рекламный щит — одна задача, одно действие.',
        'crm': 'Система управления взаимоотношениями с клиентами. Как цифровая картотека клиентов с историей общения.',
        'erp': 'Система управления ресурсами предприятия. Как цифровой мозг компании — учитывает всё.',
        'saas': 'Программное обеспечение как услуга. Как подписка на Netflix — платишь и пользуешься.',
        'pwa': 'Прогрессивное веб-приложение. Сайт, который работает как мобильное приложение.',
        'spa': 'Одностраничное приложение. Сайт, который не перезагружается при переходах.',
        'mpa': 'Многостраничное приложение. Классический сайт с отдельными страницами.',
        'mvc': 'Архитектурный паттерн: Модель-Вид-Контроллер. Как разделение обязанностей в команде.',
        'mvvm': 'Архитектурный паттерн: Модель-Вид-МодельПредставления. Улучшенная версия MVC.',
        'компонент': 'Переиспользуемый элемент интерфейса. Как Lego-блок — собрал из блоков сложный интерфейс.',
        'component': 'Переиспользуемый элемент интерфейса. Как Lego-блок — собрал из блоков сложный интерфейс.',
        'пропс': 'Данные, передаваемые в компонент. Как параметры функции.',
        'props': 'Данные, передаваемые в компонент. Как параметры функции.',
        'стейт': 'Внутреннее состояние компонента. Как память — помнит, что происходит.',
        'state': 'Внутреннее состояние компонента. Как память — помнит, что происходит.',
        'хук': 'Функция для работы с состоянием в React. Как инструмент для управления данными.',
        'hook': 'Функция для работы с состоянием в React. Как инструмент для управления данными.',
        'реакт': 'Библиотека для создания интерфейсов. Как конструктор сайтов.',
        'react': 'Библиотека для создания интерфейсов. Как конструктор сайтов.',
        'ангуляр': 'Фреймворк для создания сложных веб-приложений. Как полноценный набор инструментов.',
        'angular': 'Фреймворк для создания сложных веб-приложений. Как полноценный набор инструментов.',
        'вью': 'Прогрессивный фреймворк для создания интерфейсов. Как лёгкий и гибкий конструктор.',
        'vue': 'Прогрессивный фреймворк для создания интерфейсов. Как лёгкий и гибкий конструктор.',
        'нода': 'Среда выполнения JavaScript на сервере. Как движок, который запускает JS вне браузера.',
        'node': 'Среда выполнения JavaScript на сервере. Как движок, который запускает JS вне браузера.',
        'npm': 'Менеджер пакетов для Node.js. Как магазин приложений — скачиваешь готовые библиотеки.',
        'пакет': 'Библиотека или модуль для использования в проекте. Как готовый ингредиент для блюда.',
        'package': 'Библиотека или модуль для использования в проекте. Как готовый ингредиент для блюда.',
        'зависимость': 'Библиотека, от которой зависит твой проект. Как деталь, без которой машина не едет.',
        'dependency': 'Библиотека, от которой зависит твой проект. Как деталь, без которой машина не едет.',
        'конфиг': 'Файл настроек. Как инструкция: "работай вот так".',
        'config': 'Файл настроек. Как инструкция: "работай вот так".',
        'енв': 'Переменные окружения — настройки для разных сред (разработка, продакшен). Как разные режимы работы.',
        'env': 'Переменные окружения — настройки для разных сред (разработка, продакшен). Как разные режимы работы.',
        'продакшен': 'Рабочая версия продукта, доступная пользователям. Как магазин — открыт для покупателей.',
        'production': 'Рабочая версия продукта, доступная пользователям. Как магазин — открыт для покупателей.',
        'девелопмент': 'Версия для разработки и тестирования. Как черновик — ещё не готов для публики.',
        'development': 'Версия для разработки и тестирования. Как черновик — ещё не готов для публики.',
        'стейджинг': 'Тестовая среда, максимально похожая на продакшен. Как репетиция перед спектаклем.',
        'staging': 'Тестовая среда, максимально похожая на продакшен. Как репетиция перед спектаклем.',
        'релиз': 'Выпуск новой версии продукта. Как премьера фильма.',
        'release': 'Выпуск новой версии продукта. Как премьера фильма.',
        'версионирование': 'Присвоение номеров версиям продукта. Как нумерация глав в книге.',
        'versioning': 'Присвоение номеров версиям продукта. Как нумерация глав в книге.',
        'мажорный': 'Крупное обновление с новыми возможностями. Как новая модель автомобиля.',
        'major': 'Крупное обновление с новыми возможностями. Как новая модель автомобиля.',
        'минорный': 'Небольшое обновление с улучшениями. Как рестайлинг автомобиля.',
        'minor': 'Небольшое обновление с улучшениями. Как рестайлинг автомобиля.',
        'патч': 'Исправление ошибок. Как заплатка на одежде.',
        'patch': 'Исправление ошибок. Как заплатка на одежде.',
        'хотфикс': 'Срочное исправление критической ошибки. Как экстренный ремонт.',
        'hotfix': 'Срочное исправление критической ошибки. Как экстренный ремонт.',
        'брейкпоинт': 'Точка остановки в отладке. Как пауза в видео — можно изучить, что происходит.',
        'breakpoint': 'Точка остановки в отладке. Как пауза в видео — можно изучить, что происходит.',
        'стек': 'Набор технологий для разработки. Как набор инструментов в ящике.',
        'stack': 'Набор технологий для разработки. Как набор инструментов в ящике.',
        'фулстек': 'Разработчик, который работает и с фронтендом, и с бэкендом. Как универсальный солдат.',
        'fullstack': 'Разработчик, который работает и с фронтендом, и с бэкендом. Как универсальный солдат.',
        'дев ops': 'Практики для автоматизации разработки и развёртывания. Как конвейер на заводе.',
        'devops': 'Практики для автоматизации разработки и развёртывания. Как конвейер на заводе.',
        'ci/cd': 'Непрерывная интеграция и доставка. Как автоматическая проверка и публикация кода.',
        'пайплайн': 'Последовательность автоматических действий. Как производственная линия.',
        'pipeline': 'Последовательность автоматических действий. Как производственная линия.',
        'тесты': 'Автоматические проверки кода. Как экзамен для программы — сдаст или нет.',
        'tests': 'Автоматические проверки кода. Как экзамен для программы — сдаст или нет.',
        'юнит тесты': 'Тесты отдельных компонентов. Как проверка каждой детали по отдельности.',
        'unit tests': 'Тесты отдельных компонентов. Как проверка каждой детали по отдельности.',
        'интеграционные тесты': 'Тесты взаимодействия компонентов. Как проверка, как детали работают вместе.',
        'integration tests': 'Тесты взаимодействия компонентов. Как проверка, как детали работают вместе.',
        'e2e': 'End-to-end тесты — проверка всего приложения от начала до конца. Как полный тест-драйв автомобиля.',
        'end to end': 'End-to-end тесты — проверка всего приложения от начала до конца. Как полный тест-драйв автомобиля.',
        'мок': 'Имитация объекта для тестирования. Как муляж — выглядит как настоящий, но для тестов.',
        'mock': 'Имитация объекта для тестирования. Как муляж — выглядит как настоящий, но для тестов.',
        'стаб': 'Упрощённая версия объекта для тестирования. Как заглушка — временно заменяет настоящую деталь.',
        'stub': 'Упрощённая версия объекта для тестирования. Как заглушка — временно заменяет настоящую деталь.',
        'фикстура': 'Тестовые данные. Как пример для задачи — помогает понять, что должно получиться.',
        'fixture': 'Тестовые данные. Как пример для задачи — помогает понять, что должно получиться.',
        'ассерт': 'Проверка условия в тесте. Как вопрос: "А правда ли, что...?"',
        'assert': 'Проверка условия в тесте. Как вопрос: "А правда ли, что...?"',
        'регрессия': 'Появление старой ошибки после изменений. Как старая болезнь вернулась.',
        'regression': 'Появление старой ошибки после изменений. Как старая болезнь вернулась.',
        'депрекейтед': 'Устаревший, но ещё работающий. Как старая модель — ещё работает, но уже не выпускается.',
        'deprecated': 'Устаревший, но ещё работающий. Как старая модель — ещё работает, но уже не выпускается.',
        'легаси': 'Старый код, который всё ещё используется. Как старый дом — ещё стоит, но пора ремонтировать.',
        'legacy': 'Старый код, который всё ещё используется. Как старый дом — ещё стоит, но пора ремонтировать.',
        'документация': 'Описание того, как работает код или система. Как инструкция к технике.',
        'documentation': 'Описание того, как работает код или система. Как инструкция к технике.',
        'комментарий': 'Пояснение в коде для людей. Как заметка на полях.',
        'comment': 'Пояснение в коде для людей. Как заметка на полях.',
        'ревью': 'Проверка кода коллегой. Как редактор проверяет текст перед публикацией.',
        'review': 'Проверка кода коллегой. Как редактор проверяет текст перед публикацией.',
        'код ревью': 'Проверка кода коллегой. Как редактор проверяет текст перед публикацией.',
        'code review': 'Проверка кода коллегой. Как редактор проверяет текст перед публикацией.',
        'мердж': 'Объединение изменений в коде. Как слияние двух рек в одну.',
        'merge': 'Объединение изменений в коде. Как слияние двух рек в одну.',
        'конфликт': 'Несовместимые изменения в коде. Как два человека пытаются занять одно место.',
        'conflict': 'Несовместимые изменения в коде. Как два человека пытаются занять одно место.',
        'ребейз': 'Перенос изменений на новую базу. Как пересадка с одного поезда на другой.',
        'rebase': 'Перенос изменений на новую базу. Как пересадка с одного поезда на другой.',
        'форк': 'Копия проекта для независимой разработки. Как ответвление дороги.',
        'fork': 'Копия проекта для независимой разработки. Как ответвление дороги.',
        'клон': 'Локальная копия репозитория. Как копия файла на твоём компьютере.',
        'clone': 'Локальная копия репозитория. Как копия файла на твоём компьютере.',
        'пуш': 'Отправка изменений на удалённый сервер. Как отправка письма.',
        'push': 'Отправка изменений на удалённый сервер. Как отправка письма.',
        'пул': 'Получение изменений с удалённого сервера. Как получение письма.',
        'pull': 'Получение изменений с удалённого сервера. Как получение письма.',
        'фетч': 'Загрузка изменений без объединения. Как получение посылки, но ещё не открыл.',
        'fetch': 'Загрузка изменений без объединения. Как получение посылки, но ещё не открыл.',
        'тег': 'Метка для важной версии. Как закладка в книге.',
        'tag': 'Метка для важной версии. Как закладка в книге.',
        'бранч': 'Ветка в Git для параллельной разработки. Как параллельная дорога.',
        'branch': 'Ветка в Git для параллельной разработки. Как параллельная дорога.',
        'мастер': 'Основная ветка в Git. Как главная дорога.',
        'master': 'Основная ветка в Git. Как главная дорога.',
        'мейн': 'Основная ветка в Git (современное название). Как главная дорога.',
        'main': 'Основная ветка в Git (современное название). Как главная дорога.',
        'индекс': 'Быстрое хранилище для поиска. Как алфавитный указатель в книге.',
        'index': 'Быстрое хранилище для поиска. Как алфавитный указатель в книге.',
        'запрос': 'Команда к базе данных. Как вопрос к библиотекарю: "Дай мне эту книгу".',
        'query': 'Команда к базе данных. Как вопрос к библиотекарю: "Дай мне эту книгу".',
        'транзакция': 'Набор операций, которые выполняются вместе или не выполняются вообще. Как покупка: либо всё, либо ничего.',
        'transaction': 'Набор операций, которые выполняются вместе или не выполняются вообще. Как покупка: либо всё, либо ничего.',
        'нормализация': 'Организация данных для устранения дублирования. Как наведение порядка в шкафу.',
        'normalization': 'Организация данных для устранения дублирования. Как наведение порядка в шкафу.',
        'денормализация': 'Намеренное дублирование данных для ускорения. Как копирование важных документов на стол для быстрого доступа.',
        'denormalization': 'Намеренное дублирование данных для ускорения. Как копирование важных документов на стол для быстрого доступа.',
        'репликация': 'Копирование данных между серверами. Как синхронизация файлов между компьютерами.',
        'replication': 'Копирование данных между серверами. Как синхронизация файлов между компьютерами.',
        'шардирование': 'Разделение базы данных на части. Как разделение книги на тома.',
        'sharding': 'Разделение базы данных на части. Как разделение книги на тома.',
        'кластер': 'Группа серверов, работающих вместе. Как команда — каждый делает своё, но работают вместе.',
        'cluster': 'Группа серверов, работающих вместе. Как команда — каждый делает своё, но работают вместе.',
        'нода': 'Отдельный сервер в кластере. Как игрок в команде.',
        'node': 'Отдельный сервер в кластере. Как игрок в команде.',
        'мастер нода': 'Главный сервер в кластере. Как капитан команды.',
        'master node': 'Главный сервер в кластере. Как капитан команды.',
        'слейв нода': 'Вспомогательный сервер в кластере. Как рядовой игрок.',
        'slave node': 'Вспомогательный сервер в кластере. Как рядовой игрок.',
        'реплика': 'Копия данных на другом сервере. Как дубликат документа.',
        'replica': 'Копия данных на другом сервере. Как дубликат документа.'
    };
    
    // Умный поиск: если точного совпадения нет, ищем частичное
    var translation = dictionary[term];
    
    if (!translation) {
        // Ищем частичные совпадения
        var matches = Object.keys(dictionary).filter(function(key) {
...
';
            matches.slice(0, 5).forEach(function(match) {
                translation += '• ' + match + ': ' + dictionary[match] + '

';
            });
        } else {
            translation = 'Не нашёл точного перевода. Попробуй другой термин или уточни контекст.

Совет: попробуй написать термин по-русски или по-английски.';
        }
    }
...
function generateProjectNames() {
    ...
    // Базовые слова для комбинаторной генерации
    var bases = {
        ...
        eco: ['Green','Eco','Nature','Earth','Sustain','Pure','Clean','Fresh','Vital','Bloom','Grow','Thrive','Flourish','Renew','Restore','Balance','Harmony','Cycle','Loop','Circle','Root','Leaf','Branch','Forest','Ocean','Sky','Sun','Rain','Seed']
    };
    ...
    // Генерируем 10 уникальных комбинаций
    var generatedNames = [];
    var usedCombinations = new Set();
    
    while (generatedNames.length < 10 && usedCombinations.size < 1000) {
    ...
    resultHtml += '<div style="font-weight:bold;color:#ffd700;margin-bottom:10px">✨ Варианты названий (10 из миллионов возможных):</div>';
    resultHtml += '<div class="mut" style="font-size:11px;margin-bottom:10px">Генератор создаёт уникальные комбинации. Нажми "🔄 Ещё" для новых вариантов.</div>';
    ...
    resultHtml += '<button class="btn" style="width:100%;margin-top:10px;background:#ffd700;color:#000" onclick="generateProjectNames()">🔄 Сгенерировать ещё</button>';
    resultHtml += '</div>';
